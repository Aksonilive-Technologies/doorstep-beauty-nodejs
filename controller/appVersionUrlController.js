const AppVersionURL = require("../models/appVersionUrlModel.js");
...
exports.updateAppVersion = async (req, res) => {
  ...
  try {
    ...
    let appVersionUrl = await AppVersionURL.findOne({
      userId,
      deviceType,
      isActive: true,
      isDeleted: false,
    ...
    if (!appVersionUrl) {
      appVersionUrl = await AppVersionURL.create({
        userId,
        appVersion,
        userType,
        deviceType,
      });
      await appVersionUrl.save(); // Save the new app version record
      ...
      if (appVersionUrl.appVersion !== appVersion) {
        appVersionUrl.appVersion = appVersion; // Update the appVersion
        await appVersionUrl.save(); // Save the updated app version record
