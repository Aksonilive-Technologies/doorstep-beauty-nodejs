...
const customerServiceAccount = JSON.parse(process.env.CustomerFCMServiceAccountKey);
...
}, 'customerApp'); // Alias for customer app
...
exports.sendBookingConfirmationMessage = (token) => {
    ...
    return customerApp.messaging().send(message)
        .then((response) => {
