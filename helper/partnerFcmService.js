...
const partnerServiceAccount = JSON.parse(process.env.PartnerFCMServiceAccountKey);
...
}, 'partnerApp'); // Alias for partner app
...
exports.sendNewBookingMessage = (token) => {
    ...
    return partnerApp.messaging().send(message)
        .then((response) => {
